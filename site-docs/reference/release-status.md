...
documentation site, consumer-visible package metadata, or completion of the
owner-reviewed quickstart paraphrase gate.
...
| Resample4s integration | Alder loads the sibling `../resample4s` source composite and uses the `resample4s-core` and test-scoped `resample4s-designs` artifacts. On 2026-09-13, the current local Alder revision added stage-bound `fromDesign`; its focused 11-test adapter suite and 2-test failure-rendering suite, followed by the complete affected-module suites, passed on JVM, Scala.js, and Scala Native against Resample4s `6bc4172`. | This proves the committed local source revision binds exact-once designs to Alder's normalized child-stage seed without weakening the strict precompiled route. It is not remote or production consumer-admission evidence until pinned and rerun from that revision; Resample4s also remains `0.1.0-SNAPSHOT`. |
